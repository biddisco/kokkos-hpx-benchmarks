...
for input_1_size in "${input_1_sizes[@]}"; do
    ...
    for num_threads in "${num_threads_values[@]}"; do
        ...
        pushd Kokkos
        ./parallel_scan.host.pthreads --kokkos-threads="$num_threads" $arguments >> "$output_file"
        OMP_NUM_THREADS="$num_threads" OMP_PROC_BIND=spread OMP_PLACES=threads ./parallel_scan.host.openmp $arguments >> "$output_file"
        popd
