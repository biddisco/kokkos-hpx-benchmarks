...
for input_1_size in "${input_1_sizes[@]}"; do
    ...
    for num_threads in "${num_threads_values[@]}"; do
        ...
        pushd Kokkos
        OMP_NUM_THREADS="$num_threads" OMP_PROC_BIND=close OMP_PLACES=threads ./parallel_reduce.host.openmp $arguments >> "$output_file"
        ./parallel_reduce.host.hpx --hpx:threads="$num_threads" $arguments >> "$output_file"
        popd

        pushd HPX/build/Release
        ./parallel_reduce --hpx:threads="$num_threads" $arguments >> "$output_file"
        popd
    done
done
