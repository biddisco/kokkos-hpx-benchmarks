...
for input_1_size in "${input_1_sizes[@]}"; do
    ...
    pushd Serial
    ./parallel_reduce.host $arguments >> "$output_file"
    popd
    ...
    for num_threads in "${num_threads_values[@]}"; do
        ...
        pushd Kokkos
        ./parallel_reduce.host.pthreads --kokkos-threads="$num_threads" $arguments >> "$output_file"
        OMP_NUM_THREADS="$num_threads" OMP_PROC_BIND=spread OMP_PLACES=threads ./parallel_reduce.host.openmp $arguments >> "$output_file"
        popd

        mkdir -p HPX/build/Release
        pushd HPX/build/Release
        ./parallel_reduce --hpx:threads="$num_threads" --hpx:bind=numa-balanced $arguments >> "$output_file"
        popd
    done
done
